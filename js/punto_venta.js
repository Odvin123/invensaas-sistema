let token = localStorage.getItem('token');
        ...
        document.getElementById('movimientoVentasForm').addEventListener('submit', function (e) {
            ...
            const precio = parseFloat(precioStr) || 0;
            const descuento = parseFloat(document.getElementById('descuento_producto').value) || 0; // ✅ Ya es decimal
        ...
        function addProductToSale(producto, cantidad, precio, descuento = 0) {
            ...
            if (existingItem) {
                alert(`⚠️ El producto "${producto.descripcion}" ya está en la venta. Se agregará como nuevo item.`);
                ventaActual.push({
                    id: producto.id,
                    clave: producto.id,
        ...
        async function finalizarVentaDesdeCaja() {
            ...
            try {
                const ventaData = {
                    ...
                    pagos: pagos,
                    descuento_total: ventaActual.reduce((summ, item) => summ + (item.descuento || 0), 0)
                };
        ...
        async function handleCorteCaja() {
            ...
            try {
                ...
                if (data.success && data.datos) {
                    ...
                    alert('Error al cargar los datos del corte: ' + (data.message || 'Error desconocido'));
                }
            } catch (error) {
        ...
        function mostrarCorteCajaModal(datos) {
            ...
            document.getElementById('corte-total-ventas').textContent = `C$${parseFloat(datos.total_ventas_hoy || 0).toFixed(2)}`;

            const pagos = datos.pagos || { efectivo: 0, tarjeta: 0, transferencia: 0, credito: 0 };

            document.getElementById('corte-efectivo').textContent = `C$${parseFloat(pagos.efectivo || 0).toFixed(2)}`;
            document.getElementById('corte-tarjeta').textContent = `C$${parseFloat(pagos.tarjeta || 0).toFixed(2)}`;
            document.getElementById('corte-transferencia').textContent = `C$${parseFloat(pagos.transferencia || 0).toFixed(2)}`;
            document.getElementById('corte-credito').textContent = `C$${parseFloat(pagos.credito || 0).toFixed(2)}`;
            ...
            document.getElementById('corte-efectivo-sistema').value = parseFloat(pagos.efectivo || 0).toFixed(2);
        ...
        async function procesarCorteCaja() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    alert('Error al realizar el corte: ' + (result.message || 'Error desconocido'));
                }
            } catch (error) {
